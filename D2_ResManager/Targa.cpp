...
Targa *Targa::LoadTargaImage(const void *data, DWORD type)
{
	Targa *img = new Targa();
	if (!img)
		return 0;
...
Targa *Targa::LoadTargaImage(int width, int height, void *data)
{
	Targa *img = new Targa();
	if (!img)
		return 0;
	...
	for (int i = img->m_Header.height - 1; i >= 0; i--)
	{
		for (int j = 0; j < img->m_Header.width; j++)
		{
			...
			img->m_pData[k].b = ((unsigned char*)data)[((img->m_Header.height - i - 1) * img->m_Header.width + j) * 4];
			img->m_pData[k].g = ((unsigned char*)data)[((img->m_Header.height - i - 1) * img->m_Header.width + j) * 4 + 1];
			img->m_pData[k].r = ((unsigned char*)data)[((img->m_Header.height - i - 1) * img->m_Header.width + j) * 4 + 2];
			img->m_pData[k].a = ((unsigned char*)data)[((img->m_Header.height - i - 1) * img->m_Header.width + j) * 4 + 3];
		}
	}
	return img;
}

int Targa::SaveTargaImage(const TCHAR *filename)
{
